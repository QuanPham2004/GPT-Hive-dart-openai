...
class _ChatPageState extends State<ChatPage> {
  ...
  // Function to complete the chat based on the given prompt
  void _completeChat(String prompt) async {
    ...
    // Create a stream for the chat, completions API & fine-tune events API.
    ...
    // Listen for events in the chat stream
    chatStream.listen((chatStreamEvent) {
      ...
      if (chatResponseId == chatStreamEvent.id) {
        ...
        onMessageReceived(
            id: chatResponseId,
            message: chatResponseContent); // Callback for message received
        isAiTyping = true;
      }
    });
  }
  ...
  // It creates a new TextMessage with the received id and message, and adds it to the chat
